...
const resturant = document.querySelector(".menu");
...
function displayMenuItems(menuItems) {
  const displayMenu = menuItems.map(function (items) {
    return `<article class="arc">
    <img
      class="image"
      src=${items.img}
      alt=${items.title}
  ...
  resturant.innerHTML = displayMenu.join("");
}
...
function displayMenuButtons() {
  ...
  const btnCategory = categories.map(function (category) {
    return `<button type="button" class="btn-lunch" data-id=${category}>${category}</button>`;
  });
